...
int main( )
{
    ...
    // Load models
    /*Model tienda((char*)"Models/tienda2/tienda.obj");*/
    Model carro((char*)"Models/carro/car.obj");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model = glm::mat4(1.0f); // Reiniciar la matriz de modelado
        model = glm::translate(model, glm::vec3(2.5f, 0.0f, -1.0f)); // Ajustar la altura según sea necesario
        model = glm::scale(model, glm::vec3(0.01f)); // Escalar para cubrir más área
        glUniformMatrix4fv(glGetUniformLocation(shader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
        piso.Draw(shader);

        model = glm::mat4(1.0f); // Reiniciar la matriz de modeladossssssssssssssssssss
        model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f)); // Ajustar la altura según sea necesario
        model = glm::scale(model, glm::vec3(0.01f)); // Escalar para cubrir más área
        glUniformMatrix4fv(glGetUniformLocation(shader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
        edif.Draw(shader);
